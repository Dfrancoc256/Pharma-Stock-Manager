...
export function registerSheetsRoutes(app: Express) {
    ...
    try {
      ...
      const detalle = detalleRows
        ...
        .map((r: string[]) => ({
          ...
          productoId: String(r[1] || '').trim(),
          producto: r[2] || '',
          tipoPrecio: r[3] || '',
          cantidad: toInt(r[4]),
          precioVendido: toNumber(r[5]),
  ...
  app.get('/api/sheets/dashboard', async (req, res) => {
    try {
      ...
      const montoVentasHoy = ventasHoyRows.reduce((acc: number, v: string[]) => acc + toNumber(v[7]), 0);
      ...
      const detallesPorVentaHoy: Record<string, { nombre: string; cantidad: number; precioUnitario: number; subtotal: number }[]> = {};
