    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/50 backdrop-blur-sm p-0 md:p-4">
      <div className="bg-white w-full md:max-w-lg rounded-t-3xl md:rounded-3xl overflow-hidden shadow-2xl animate-in slide-in-from-bottom-4 md:zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
        <div className="flex items-start gap-3 p-5 border-b bg-gradient-to-r from-violet-50 to-primary/5">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-xl font-extrabold text-primary">
                Q {parseFloat(producto["Precio unidad"] || "0").toFixed(2)}
              </span>
              {tieneColumnaStock && (
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                    sinStock ? "bg-red-100 text-red-600" : "bg-green-100 text-green-700"
                  }`}
                >
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl hover:bg-black/5 flex-shrink-0"
            data-testid="button-cerrar-info"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {error && (
            <div className="m-4 p-4 bg-red-50 border border-red-200 rounded-2xl text-sm text-red-700 flex items-center gap-2">
              ...
              <button onClick={cargarInfo} className="ml-auto text-red-600 underline text-xs">
                Reintentar
              </button>
            </div>
          )}
            ...
            <div className="p-5 space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4">
                ...
                {info.duracionTratamiento && (
                  <p className="text-xs text-blue-600 mt-1">Duración: {info.duracionTratamiento}</p>
                )}
              </div>
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {info.recomendaciones.map((rec, i) => (
                      <div
                        key={i}
                        className="flex items-start gap-2 text-sm text-muted-foreground bg-muted/40 rounded-xl px-3 py-2"
                      >
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {info.contraindicaciones.map((c, i) => (
                      <div
                        key={i}
                        className="flex items-start gap-2 text-xs text-orange-800 bg-orange-50 rounded-xl px-3 py-2"
                      >
...
export default function POSPage() {
  ...
  const handleCheckout = async () => {
    ...
    if (tipo === "fiado" && fiadorModo === "existente" && !fiadorId) {
      alert("Selecciona un fiador");
      return;
    }
    if (tipo === "fiado" && fiadorModo === "nuevo" && !nuevoFiadorNombre.trim()) {
      alert("Ingresa el nombre del fiador");
      return;
    }
    if (tipo === "contado" && (!amountPaid || parseFloat(amountPaid) < cartTotal)) {
    ...
    const fiadorSeleccionado =
      tipo === "fiado" && fiadorModo === "existente"
    ...
    createVenta.mutate({
      cliente:
        tipo === "fiado"
          ? fiadorModo === "nuevo"
            ? nuevoFiadorNombre.trim()
            : fiadorSeleccionado?.Nombre || ""
          : clienteNombre || "Contado",
      tipo,
  ...
  const handleSendEmail = async (subject: string, body: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const mailLink = `mailto:${shareEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(
    ...
    <Layout>
      <div className="flex gap-6 h-[calc(100vh-4rem)]">
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {aiError && (
            <div className="mb-2 px-3 py-2 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700">
              {aiError}
            </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-3">
                ...
                <button onClick={clearSearch} className="text-sm text-violet-600 underline">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 pb-4">
                {(aiResultados === null
                  ...
                  return (
                    <div
                      key={p.ID}
                      className={`relative group bg-white rounded-2xl border border-border/60 flex flex-col gap-1 overflow-hidden ${
                        sinStock ? "opacity-40" : "hover:border-primary/50 hover:shadow-xl hover:shadow-primary/10"
                      }`}
                    >
                      <button
                      ...
                      >
                        <div className="text-xs text-muted-foreground font-mono bg-accent/50 w-max px-2 py-0.5 rounded-md">
                          {p.ID}
                        </div>
                        <div className="font-bold text-foreground line-clamp-2 leading-tight text-sm group-hover:text-primary">
                          {p.Nombre}
                        </div>
        ...
        <div className="w-80 glass-card rounded-3xl flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {cart.length === 0 ? (
              ...
              cart.map((item) => (
                <div
                  key={`${item.producto.ID}-${item.tipoPrecio}`}
                  className="bg-white p-3 rounded-2xl border border-border shadow-sm"
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() => removeFromCart(item.producto.ID, item.tipoPrecio)}
                  ...
                  <div className="flex gap-1 mb-2">
                    {(["unidad", "blister", "caja"] as TipoPrecio[]).map((t) => {
                      ...
                      return (
                        <button
                          key={t}
                          onClick={() => updateTipoPrecio(item.producto.ID, item.tipoPrecio, t)}
                          className={`text-xs px-2 py-0.5 rounded-lg font-medium interactive-btn border ${
                            item.tipoPrecio === t
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 bg-muted/50 p-1 rounded-xl">
                      <button
                        onClick={() => updateQty(item.producto.ID, item.tipoPrecio, -1)}
                        className="p-1 hover:bg-white rounded-lg interactive-btn"
                      >
                        <Minus size={14} />
                      </button>
            ...
            <div className="space-y-5">
                ...
                <div className="flex gap-2">
                  {(["contado", "fiado"] as const).map((t) => (
                    <button
                      ...
                      type="button"
                      onClick={() => setTipo(t)}
                      className={`flex-1 py-2.5 rounded-xl font-bold border-2 interactive-btn ${
                        tipo === t ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground"
                      }`}
                    >
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  ...
                  className="flex-1 py-3 rounded-xl font-bold bg-gradient-to-r from-primary to-teal-500 text-white interactive-btn shadow-lg shadow-primary/25"
                >
          ...
          <div className="glass-card rounded-3xl w-full max-w-md animate-in zoom-in-95 duration-200 overflow-hidden">
            <div className="flex items-center gap-3 p-6 border-b bg-gradient-to-r from-green-50 to-emerald-50">
              ...
              <button
                ...
                data-testid="button-cerrar-share"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl font-bold text-sm flex items-center gap-2 interactive-btn text-white ${
                      shareWhatsapp.replace(/\D/g, "")
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <button
                    onClick={() => handleSendEmail("Recibo Farmacia Web", "Adjunto recibo de compra")}
