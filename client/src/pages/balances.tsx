...
export default function BalancesPage() {
  ...
  const { data, isLoading } = useQuery<BalancesData>({
    ...
    queryFn: async () => {
      ...
      if (json?.data) {
        return {
          ingresos: String(json.data.ingresos ?? "0"),
          egresos: String(json.data.egresos ?? "0"),
          cajaNeta: String(json.data.cajaNeta ?? "0"),
          movimientos: Array.isArray(json.data.movimientos) ? json.data.movimientos : [],
        };
      }

      return {
        ingresos: String(json?.ingresos ?? "0"),
        egresos: String(json?.egresos ?? "0"),
        cajaNeta: String(json?.cajaNeta ?? "0"),
    ...
    <Layout>
      <PageHeader
        ...
        action={
          <button
            ...
            data-testid="button-nuevo-movimiento"
          >
      ...
      {selectedMov && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setSelectedMov(null);
          }}
        >
          <div className="glass-card rounded-3xl w-full max-w-md max-h-[85vh] overflow-y-auto animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-6 border-b border-border/30">
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-2xl flex items-center justify-center ${
                    selectedMov.tipo === "ingreso" ? "bg-green-100" : "bg-red-100"
                  }`}
                >
              ...
              <button
                ...
                data-testid="button-close-detalle"
              >
                <X size={18} />
              </button>
            </div>
                  ...
                  <div className="rounded-2xl border border-border/40 overflow-hidden">
                    <table className="w-full text-sm">
                      ...
                      <tfoot className="bg-muted/30 border-t border-border/40">
                        <tr>
                          <td colSpan={3} className="px-3 py-2 font-bold text-sm">
                            Total
                          </td>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 py-2 rounded-xl font-bold border-2 interactive-btn ${
                      tipo === "ingreso"
                        ? "border-green-500 bg-green-50 text-green-700"
                        : "border-border text-muted-foreground"
                    }`}
                    data-testid="button-tipo-ingreso"
                  >
                    Ingreso
                  </button>
                  <button
                    ...
                    data-testid="button-tipo-egreso"
                  >
                    Egreso
                  </button>
                </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  ...
                  data-testid="button-guardar-movimiento"
                >
